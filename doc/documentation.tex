...
\begin{abstract}
This document serves as documentation for the interpreter as well as an introduction to Befunge-93. We describe how to run the interpreter and give a comprehensive list of all supported instruction characters. We give examples of Befunge programs and discuss the implementation, specifically data structures and algorithms. Finally we mention shortcomings and caveats of the interpreter.
\end{abstract}
...
\code{"egnufeB">:\#v\_@}\\
\code{\hspace{3.5em}\^{} ,<}
\caption{Printing a string in Befunge. The program prints \code{Befunge}. Note that the string is pushed reversely onto the stack and must end with a 0 on the stack.}
\label{fig:print}
\end{figure}
